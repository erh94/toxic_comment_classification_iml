...
model_json = model.to_json()
with open("./models/model_GRU"+suffix+".json", "w") as json_file:
    json_file.write(model_json)
model.save_weights("./models/model_LSTM"+suffix+".h5")
print("Saved model to disk")
...
json_file = open("./models/model_LSTM"+suffix+".json", 'r')
loaded_model_json = json_file.read()
json_file.close()
loaded_model = model_from_json(loaded_model_json)
loaded_model.load_weights("./models/model_LSTM"+suffix+".h5")
print("Loaded model from disk")
# In[ ]:
...
submission.to_csv('./submission/submission_GRU_glove_300_epoch3.csv', index=False)
